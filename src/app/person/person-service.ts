...
export class PersonService {
  ...
  async getPerson(id: string | number): Promise<Person> {
    ...
    const person = this.mockPeople.find(p => p.id.toString() === id.toString());
    if (!person) {
  ...
  async updatePerson(person: Person): Promise<Person> {
    ...
    const index = this.mockPeople.findIndex(p => p.id.toString() === person.id.toString());
    if (index === -1) {
      throw new Error('Person not found');
    }
  ...
  async deletePerson(id: string | number): Promise<void> {
    ...
    const index = this.mockPeople.findIndex(p => p.id.toString() === id.toString());
    if (index === -1) {
      throw new Error('Person not found');
    }
